...
void RedBlackTree<TKey, TData>::rb_delete(RBNode* z_node, std::stack<RBNode**>& way)
{
	...
	if (root == nullptr) // parent_x_node and x_node are nullptr. “о есть этот случай отрезает наш, возможно, пустой стек дальше.
	{
		return;
	}
	...
	while (x_node != root && x_color == BLACK) //if x_node != root --> we have parent for x.
	{
		...
		if (x_node == parent_x_node->left)
		{
			...
			if (w_node_color== RED)
			{
				...
				if (x_node != nullptr)
				{
					findWay(x_node->key, current_way); // после поворота у нас уже может быть другой путь до корн€. из-за case 1 (left_rotate)
...
void RedBlackTree<TKey, TData>::deleteHook(TKey key, RBNode* node)
{
	...
	if (comparator->compare(key, node->key) == 0)
	{
		...
		//надо удалить это. ѕеренести элемент из списка в нодуы
	}
	else
	{
